...
def main()->int:
    p=argparse.ArgumentParser(); p.add_argument("root",type=Path); p.add_argument("--course",required=True); p.add_argument("--target",required=True); args=p.parse_args()
    ...
    config={"schema_version":"1.1","course":args.course,"target":args.target,"run_slug":run,"output":f"out/{course_slug}/{target_slug}.md","provenance":f"out/{course_slug}/{target_slug}.sources.json"}
